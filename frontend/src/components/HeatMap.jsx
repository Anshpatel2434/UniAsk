...
const HeatMap = ({ doubts, solutions }) => {
  ...
  useEffect(() => {
    const dates = doubts.map((doubt) => {
      return doubt.postedOn.split("T")[0];
    });
    const dates1 = solutions.map((solution) => {
      return solution.postedOn.split("T")[0];
    });
    const combinedDates = [...dates, ...dates1];
    const data1 = {};

    for (let i = 0; i < combinedDates.length; i++) {
      let ele = combinedDates[i];
      if (data1[ele]) {
        data1[ele] += 1;
      } else {
        data1[ele] = 1;
